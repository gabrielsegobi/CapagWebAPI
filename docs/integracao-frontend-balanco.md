...
**Regra das fórmulas:** igual ao plano de contas — **crédito (azul) → positivo**, **débito → negativo**. Não inverter o Ativo de novo (o D/C do balanço já vem invertido em relação à DRE). Exceção: `3.01.01` e filhos em magnitude. **PMP** e Cobertura de Juros (ICP) usam magnitude em todos os códigos da equação. **Ciclo Financeiro** compõe PME+PMR−PMP via tags (`{@PME}` etc. no JSON), sem recalcular contas. **ROE** com PL (`2.03`) ≤ 0: `valor` nulo e `alerta`/`mensagem` = `"Não Analisar: Informação Comprometida"`. Demais indicadores calculam com o PL negativo.
...
Os indicadores Capag usam a consolidação de `GetDClByAnoAndCodigoHandler` com sinal econômico. Exceções de magnitude (PMP/CJ) são aplicadas na avaliação da fórmula, não na consolidação. Ciclo Financeiro referencia PME/PMR/PMP via `{@...}`.
